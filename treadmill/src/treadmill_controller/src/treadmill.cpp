    ...
    public:
        TreadmillController(hid_device* m_hid) : Node("treadmill") {
            ...
            subscription_ = this->create_subscription<geometry_msgs::msg::Twist>(
                "cmd_vel", 10, std::bind(&TreadmillController::speed_callback, this, _1)
            );
...
//                RCLCPP_INFO(this->get_logger(), "D: %i C: %f T: %f", diff, change_w, step_time);
        ...
        void speed_callback(const geometry_msgs::msg::Twist & message) {
            last_x_vel  = message.linear.x;

            const int rpm = (int) abs(600*message.linear.x); //Treadmill units in m/min with fixed point, 500 = 50.0m/min

            uint8_t data0[10];
            data0[0] = 0;
            data0[1] = rpm & 0xff;
            data0[2] = (rpm & 0xff00) >> 8;
            data0[3] = 0;
            data0[4] = 0;
            data0[5] = 0;
            data0[6] = 0;
